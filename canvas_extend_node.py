...
class CanvasExtendNode:
    ...
    RETURN_NAMES = ("output_image", "mask", "image_mask", "extend_data")
    ...
    def process_canvas(self, image, extend_data, fill_color):
        ...
        # 掩码画布尺寸：保持与原图一致（不考虑扩展）
        mask_canvas_width = original_width
        mask_canvas_height = original_height
        ...
        # 生成掩码（扩展区域=白，原图区域=黑，向后兼容的旧逻辑）
        mask_image = self.generate_mask(output_width, output_height, left, right, top, bottom)

        # 生成裁切框掩码（裁切框=白，周围=黑；画布尺寸=原图尺寸，不考虑扩展）
        image_mask_image = self.generate_image_mask(
            mask_canvas_width, mask_canvas_height, left, right, top, bottom, original_width, original_height
        )

        # 转换回tensor
        result_tensor = self.pil_to_tensor(output_image)
        mask_tensor = self.pil_to_mask_tensor(mask_image)
        image_mask_tensor = self.pil_to_mask_tensor(image_mask_image)

        return (result_tensor, mask_tensor, image_mask_tensor, extend_data)
    ...
    def generate_image_mask(self, canvas_width, canvas_height, left, right, top, bottom, image_width, image_height):
        """
        生成裁切框掩码：
        - 画布尺寸 = canvas_width × canvas_height（由调用方传入，这里 = 原图尺寸）
        - 裁切框在原图坐标系中的位置：
            x ∈ [-left, image_width + right]
            y ∈ [-top, image_height + bottom]
        - 裁切框矩形 = 白色（255）
        - 其余区域 = 黑色（0）
        """
        ...
        x0 = int(round(-left))
        y0 = int(round(-top))
        x1 = int(round(image_width + right))
        y1 = int(round(image_height + bottom))
        ...
        x0c = max(0, x0)
        y0c = max(0, y0)
        x1c = min(canvas_width, x1)
        y1c = min(canvas_height, y1)

        if x1c > x0c and y1c > y0c:
            draw.rectangle([x0c, y0c, x1c, y1c], fill=255)
