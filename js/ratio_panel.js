    ...
    async beforeRegisterNodeDef(nodeType, nodeData, app) {
        if (nodeData.name === "RatioCropWithPanel") {
            ...
            nodeType.prototype.onWidgetChanged = function(name, value, old_value, widget) {
                ...
                // 当比例、模式或尺寸变化时，重新计算裁剪框并重绘
                if ((name === "aspect_ratio" || name === "crop_size" || name === "mode" || name === "custom_width" || name === "custom_height") && this.panel) {
...
Object.assign(LGraphNode.prototype, {
    ...
    addRatioPanel() {
        ...
        controlPanel.innerHTML = `
            ...
            <div style="flex: 1; min-width: 100px;">模式: <span id="cropMode">按比例</span></div>
        `;
        ...
        // 活动指针 id（用于 pointer 事件）
        this.panel.activePointerId = null;
    ...
    bindRatioPanelEvents(area, canvas) {
        ...
        // 指针按下 - 移动裁剪框（使用 pointer 事件以便更稳健地捕获拖动）
        canvas.addEventListener('pointerdown', (e) => {
            ...
            // 逻辑像素坐标（不乘以dpr）
            const logicX = e.clientX - rect.left;
            const logicY = e.clientY - rect.top;

            // 计算图像显示参数（与 drawCanvas 中一致）
            ...
            // 将画布坐标转换为图像坐标
            const imageX = (logicX - imgX) / baseScale;
            const imageY = (logicY - imgY) / baseScale;

            // 获取裁剪框
            const cropBox = this.getCropBoxRect();
            if (!cropBox) return;

            // 允许较大的容差以便点击到线框（画布像素容差）
            const hitMarginPx = 12;
            const hitMarginImage = hitMarginPx / baseScale;

            const hit =
                imageX >= (cropBox.x - hitMarginImage) && imageX <= (cropBox.x + cropBox.width + hitMarginImage) &&
                imageY >= (cropBox.y - hitMarginImage) && imageY <= (cropBox.y + cropBox.height + hitMarginImage);

            if (hit) {
                // 直接进入框拖动模式（移除中心圆点判定，避免其干扰）
                panel.draggingMode = 'box';

                panel.isDragging = true;
                // 记录激活指针 id
                panel.activePointerId = e.pointerId;
                panel.lastX = e.clientX;
                panel.lastY = e.clientY;
                // pointer capture to ensure we keep receiving pointer events
                try { canvas.setPointerCapture && canvas.setPointerCapture(e.pointerId); } catch (err) {}
                area.style.cursor = 'grabbing';
                console.log('[RatioPanel] mousedown hit:', { imageX, imageY, mode: panel.draggingMode, baseScale });
                e.preventDefault();
                e.stopPropagation();
            }
        });
        ...
        const mouseMoveHandler = (e) => {
            // 如果 activePointerId 存在，仅允许该指针驱动拖拽
            if (panel.activePointerId != null && e.pointerId !== panel.activePointerId) return;
            ...
            console.log('[RatioPanel] mousemove', { dx, dy, imageDx, imageDy, panelImageScale: panel.imageScale });
            
            // 更新裁剪框位置
            if (panel.draggingMode === 'dot' || panel.draggingMode === 'box') {
        ...
        const mouseUpHandler = (e) => {
            // 只响应激活指针的释放（或当没有设置 activePointerId 时都响应）
            if (panel.activePointerId != null && e.pointerId !== panel.activePointerId) return;
            if (!panel.isDragging) return;
            
            panel.isDragging = false;
            panel.draggingMode = null;
        ...
        // 使用 pointer 事件来捕获拖拽（与 pointerdown 一致）
    ...
    calculateCropBoxSize(aspectRatio, cropSize, scale) {
        // 读取模式和自定义尺寸（若存在）
        const mode = this.widgets?.find(w => w.name === "mode")?.value || "按比例";
        const customW = this.widgets?.find(w => w.name === "custom_width")?.value || 512;
        const customH = this.widgets?.find(w => w.name === "custom_height")?.value || 512;
        ...
        if (mode === "自定义尺寸") {
            // 使用自定义宽高（以图像像素为单位），再乘以缩放
            width = customW * scale;
            height = customH * scale;
        } else {
            // 按比例模式
            if (aspectRatio === "自定义") {
    ...
    updateInfo() {
        ...
        const modeSpan = panel.controlPanel.querySelector('#cropMode');
