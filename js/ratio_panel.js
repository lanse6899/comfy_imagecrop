...
Object.assign(LGraphNode.prototype, {
    ...
    bindRatioPanelEvents(area, canvas) {
        ...
        // 鼠标拖拽 - 移动裁剪框（改为在此处直接计算坐标并判断，提高可靠性）
        canvas.addEventListener('mousedown', (e) => {
            ...
            // 也支持基于画布坐标的直接命中检测：把裁剪框转换到画布坐标，检查点击是否在其附近
            const cropCanvasX = imgX + cropBox.x * baseScale;
            const cropCanvasY = imgY + cropBox.y * baseScale;
            const cropCanvasW = cropBox.width * baseScale;
            const cropCanvasH = cropBox.height * baseScale;
            const hitMarginCanvas = 12; // 画布像素容差

            const hitCanvas = logicX >= (cropCanvasX - hitMarginCanvas) && logicX <= (cropCanvasX + cropCanvasW + hitMarginCanvas) &&
                              logicY >= (cropCanvasY - hitMarginCanvas) && logicY <= (cropCanvasY + cropCanvasH + hitMarginCanvas);
        ...
        const mouseMoveHandler = (e) => {
            ...
            // 重新计算当前图像显示缩放（以防 panel.imageScale 不一致）
            const canvasWidth = panel.canvas.width / (panel.dpr || 1);
            const canvasHeight = panel.canvas.height / (panel.dpr || 1);
            const img = panel.inputImage;
            if (!img) return;
            ...
            // 将像素移动量转换为图像坐标系偏移
            const imageDx = dx / baseScale;
            const imageDy = dy / baseScale;
