        ...
        if (nodeData.name === "ImageAnnotateWithPanel") {
            ...
            nodeType.prototype.drawAnnotations_internal = function(ctx, annotations, imageRect) {
                ...
                annotations.forEach((point, index) => {
                    ...
                    // 绘制地图标记样式
                    this.drawMapMarker_internal(ctx, screenX, screenY, displaySize, fillColor, label, txtColor, isSelected);
                });
            };
            
            nodeType.prototype.drawMapMarker_internal = function(ctx, x, y, size, fillColor, label, textColor, isSelected) {
                const radius = size / 2;
                ...
                // 使用支持中文的字体，按优先级尝试
                ctx.font = `bold ${Math.floor(size * 0.5)}px "Microsoft YaHei", "微软雅黑", "SimHei", "黑体", Arial, sans-serif`;
                ctx.textAlign = 'center';
