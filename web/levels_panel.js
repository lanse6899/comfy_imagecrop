            ...
            if (previewRect && panel.inputImage) {
                try {
                    ...
                    try {
                        ...
                        // Pipeline: first apply RGB overall LUT, then apply per-channel LUTs on top.
                        ...
                        for (let k=0;k<data.length;k+=4) {
                            ...
                            // apply RGB first
                            const r_after_rgb = lutRGB[r0];
                            const g_after_rgb = lutRGB[g0];
                            const b_after_rgb = lutRGB[b0];
                            // then apply per-channel LUTs
                            data[k]   = lutR[r_after_rgb];
                            data[k+1] = lutG[g_after_rgb];
                            data[k+2] = lutB[b_after_rgb];
                        }
