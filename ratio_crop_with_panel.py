...
class RatioCropWithPanel:
    ...
    def crop_image(self, image, aspect_ratio, crop_size, mode="按比例", custom_width=512, custom_height=512, crop_x=0, crop_y=0, crop_scale=1.0):
        """
        执行图像剪裁并生成预览
        图像固定，只移动和缩放裁剪框
        """
        ...
        # 计算裁剪框尺寸（根据模式：按比例 或 自定义尺寸）
        if mode == "自定义尺寸":
            # 使用用户指定的自定义尺寸，应用缩放因子
            crop_width = int(custom_width * crop_scale)
            crop_height = int(custom_height * crop_scale)
        else:
            # 按比例模式，使用预设比例或自定义比例选项
            if aspect_ratio == "自定义":
                # 当选择自定义比例但仍在按比例模式时，使用crop_size作为正方形基准
