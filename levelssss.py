...
class LevelsPanel:
    ...
    def build_luts_from_params(self, params, channel):
        ...
        # Photoshop 风格通常是先对各通道处理再对 RGB 做复合，这里保持简单：如果 channel=="RGB" 则使用 lut_rgb，否则对指定通道使用对应 lut，其它通道使用单位映射或对应 lut
        if channel == "RGB":
            return lut_rgb, lut_rgb, lut_rgb
        elif channel == "R":
            return lut_r, lut_g if lut_g is not None else np.arange(256,dtype=np.uint8), lut_b if lut_b is not None else np.arange(256,dtype=np.uint8)
        elif channel == "G":
            return lut_r if lut_r is not None else np.arange(256,dtype=np.uint8), lut_g, lut_b if lut_b is not None else np.arange(256,dtype=np.uint8)
        else:  # B
            return lut_r if lut_r is not None else np.arange(256,dtype=np.uint8), lut_g if lut_g is not None else np.arange(256,dtype=np.uint8), lut_b
