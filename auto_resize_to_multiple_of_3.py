...
def _round_values_3(n: int):
    lower = (n // 3) * 3
    upper = lower if n % 3 == 0 else lower + 3
...
class AutoResizeTo3:
    ...
    DESCRIPTION = "将图片调整到宽高均可被 3 整除，可选拉伸/裁剪/填充策略"

    def _process_one(self, img: torch.Tensor, mode: str):
        # img: HWC, float32 0-1
        h, w, _ = img.shape
        lower_w, upper_w, nearest_w = _round_values_3(w)
        lower_h, upper_h, nearest_h = _round_values_3(h)
        ...
        if mode == "center_crop":
            ...
            # 兜底：若仍未整除则再填充到上取整
            if target_w % 3 != 0 or target_h % 3 != 0:
                target_w = math.ceil(target_w / 3) * 3
                target_h = math.ceil(target_h / 3) * 3
...
NODE_CLASS_MAPPINGS = {"AutoResizeTo3": AutoResizeTo3}
NODE_DISPLAY_NAME_MAPPINGS = {"AutoResizeTo3": "🔵被3整除像素处理"}
